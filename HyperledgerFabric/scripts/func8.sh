...
installChaincode () {
	PEER=$1
	setGlobals $PEER
	peer chaincode install -n $CC_NAME -v 1.0 -p github.com/hyperledger/fabric/examples/chaincode/go/chaincode_example07 >&log.txt
	res=$?
	cat log.txt
...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		echo "preparing to instantiate $CHANNEL_NAME and $CC_NAME"
		...
		echo "preparing to instantiate 2nd $CORE_PEER_TLS_ENABLED and $CHANNEL_NAME and $CC_NAME and $ORDERER_CA"
...
add () {
	...
	# lets supply it directly as we know it using the "-o" option
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode invoke -o orderer.example.com:7050 -C $CHANNEL_NAME -n $CC_NAME -c '{"Args":["invoke","x","y","worldChampion"]}' >&log.txt
	else
		peer chaincode invoke -o orderer.example.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n $CC_NAME -c '{"Args":["invoke","x","y","worldChampion"]}' >&log.txt
	fi
	res=$?
	cat log.txt
	verifyResult $res "Invoke execution on PEER$PEER failed "
